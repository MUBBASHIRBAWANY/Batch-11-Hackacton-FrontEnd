...
if(isLogin == 'Admin'){
  ...
  const getdata = async ()=>{
      ...
      const res = await axiosInstance.get("https://mern-blogs-steel.vercel.app/users")
    
    return dispatch(fetchUsers(res.data.data))
  }
const navigate = useNavigate()
  ...
  const deleteData = async (id)=>{
      ...
      const res = await axiosInstance.delete(`https://mern-blogs-steel.vercel.app/users/user/${id}`)
      console.log(res)
        ...
        <table className="min-w-full max-w-full bg-white rounded-lg shadow-md">
          <thead>
            <tr className="bg-blue-400 text-white uppercase text-xs md:text-sm leading-normal">
              ...
              <th className="py-3 px-4 md:px-6 text-left">IsAdmin </th>
