...
import { Admin, AsaUser, Hod } from '../../Redux/Reducers/LoginerReducer';
import { createData } from '../../Api/CRUD Functions';
...
const dispatch = useDispatch()
    const valideUser = async () =>{
        ...
        try{
            ...
            if (response.data.val.userType == 2){
                console.log("User")
                dispatch(AsaUser())
            }
            else if(response.data.val.userType == 1){
                console.log("Admin")
                dispatch(Admin())
            }
            else if(response.data.val.userType == 3){
                console.log("Hod")
                dispatch(Hod())
            }
